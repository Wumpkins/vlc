...
let rec generate_ptx_literal literal = 
  let l = match literal with
    ...
    | Ptx_Array_Access(l,l_list) -> generate_ptx_literal l ^ "[" ^
...
let generate_ptx_register_declaration rdecl = 
  let reg_string = 
    ".reg " ^ generate_ptx_data_type rdecl.reg_type ^ " %" ^ rdecl.reg_id ^ "<" ^
      string_of_int rdecl.num_registers ^">;"
...
let generate_ptx_vdecl vdecl = 
  let vdecl_string = 
    ...
    | Ptx_Vdecl (ss, vtype, id) -> 
       (match vtype with 
       | Ptx_Primitive(p) -> generate_ptx_state_space ss ^ "      " ^
	  generate_ptx_data_type p ^ " " ^ generate_id id 
...
let rec generate_ptx_statement statement =
  let s = match statement with
    | Ptx_Load(ss, d, id1, id2) -> "ld" ^ generate_ptx_state_space(ss) ^
       generate_ptx_variable_type(d)
       ^ "     " ^ generate_ptx_literal id1 ^ ",[" ^ generate_ptx_literal id2 ^ "];\n"
    | Ptx_Store(ss, d, id1, id2) -> "st" ^ generate_ptx_state_space(ss) ^
       generate_ptx_variable_type(d)
       ^ "     " ^ "[" ^ generate_ptx_literal id1 ^ "]," ^ generate_ptx_literal id2 ^ ";\n"
    | Ptx_Move(d, id1, id2) -> "mov" ^ generate_ptx_variable_type(d) ^ "     " ^
       generate_ptx_literal id1 ^ ", " ^ generate_ptx_literal id2 ^ ";\n"
    | Ptx_Binop(o, t, id1, id2, id3) -> generate_ptx_binary_operator(o) ^
       generate_ptx_variable_type(t) 
       ^ "     " ^ generate_ptx_literal id1 ^ ", " ^ generate_ptx_literal id2 ^ ", " 
    ...
    | Ptx_Empty_Call(id, vlist) -> "call " ^ Utils.idtos(id) ^"(" ^
       (generate_list generate_ptx_literal "," vlist)^")" ^ ";\n"
...
let generate_ld_statement vdecl =
  let ld_string =  
    match vdecl with 
    | Ptx_Vdecl(ss,vtype,id) -> "ld" ^ (generate_ptx_state_space ss) ^
...
let generate_load_statement vdecl = 
  ...
  | Sast.Ptx_Vdecl(ss,vtype,id) -> 
     match ss with 
     | Sast.Param -> "ld" ^ (generate_ptx_state_space ss) ^
	(generate_ptx_variable_type vtype) ^ " " ^
        id.reg_name ^  string_of_int id.reg_num^ "," ^ 
        "[" ^ Utils.idtos id.var_name ^ "];"
     | _ ->
        (match vtype with 
        | Sast.Ptx_Primitive(p) -> "ld" ^ (generate_ptx_state_space ss) ^
	   (generate_ptx_data_type p ) ^ 
           " " ^ id.reg_name ^ string_of_int id.reg_num ^ "," ^ Utils.idtos id.var_name 
        | Sast.Ptx_Array(t,n) -> "ld" ^ (generate_ptx_state_space ss) ^
	   (generate_ptx_variable_type vtype) ^ 
           " %" ^ id.reg_name ^  string_of_int id.reg_num^ "," ^ 
           "[" ^ Utils.idtos id.var_name ^ "];"
        )
...
let generate_ptx_function f =
  let ptx_function_body = 
    ".visible " ^ generate_ptx_function_type(f.ptx_fdecl_type) ^ 
      " (" ^ generate_ptx_param f.ptx_fdecl_return_param ^ ") " ^
      (Utils.idtos f.ptx_fdecl_name) ^ 
...
let generate_array_load vdecl a_start nr_start type_start num_arr_param = 
  let cvt_string = 
    match vdecl with 
    | Ptx_Vdecl(ss,vtype,id) -> 
       "cvta.to.global.64    %ptr" ^ string_of_int nr_start ^  ",%ptr" ^
	 string_of_int a_start ^ ";\n" ^ 
         "add.s64              %ptr" ^ string_of_int (nr_start +1) ^ ",%ptr" ^
	 string_of_int nr_start ^ ", %ptr" ^ string_of_int (num_arr_param+1) ^ ";\n" ^ 
         "ld.global" ^ (generate_ptx_variable_type vtype) ^ "        %vlc" ^
	 string_of_int type_start ^ ",[%ptr"^ string_of_int nr_start^ "];\n\n"
  ...
  let cvt_string = 
    ...
    | Ptx_Vdecl(ss,vtype,id) -> 
	 ...
	 string_of_int a_start ^ ";\n" ^ 
         "add.s64              %ptr" ^ string_of_int (nr_start +1) ^ ",%ptr" ^
	 string_of_int nr_start ^ ", %ptr" ^ string_of_int (num_arr_param+1) ^ ";\n" ^ 
         "ld.global" ^ (generate_ptx_variable_type vtype) ^ "        %rtype" ^
	 string_of_int type_start ^ ",[%ptr"^ string_of_int nr_start^ "];\n\n"
  in sprintf "%s" cvt_string
...
let rec generate_load_and_access_elements vdecl_list array_start nex_reg_start type_start num_arr_param final_string= 
  let s =  match vdecl_list with 
    | [] -> final_string 
    | hd::tl -> 
       let statement = generate_array_load hd array_start nex_reg_start
	 type_start num_arr_param in
       generate_load_and_access_elements tl (array_start+1) (nex_reg_start+3) (type_start + 1)
	 num_arr_param (final_string ^ statement)
  in sprintf "%s" s
...
let rec generate_call_params num_arr_param final_string= 
  ...
  | _ -> generate_call_params (num_arr_param -1) (final_string ^
						    (generate_call_param num_arr_param ^ ",")))
  ...
  | "map" -> 
     (let ptx_function_string = 
	  ...
	  (List.rev(hof.ptx_return_array_info::(List.rev hof.ptx_input_arrays_info))) ^ "\n\n" ^ 
          ...
          (* Load input arrays *)
          generate_array_rtype hof.ptx_return_array_info 1 (num_arr_param +2)
	  1 num_arr_param ^"\n"^
          generate_load_and_access_elements hof.ptx_input_arrays_info (2)
	  (num_arr_param+5) type_start num_arr_param "" ^ "\n" ^ 
          "call " ^ "(%rtype1)," ^ (Utils.idtos hof.ptx_applied_kernel_function) ^
	  ",(" ^ generate_call_params (num_arr_param-1) "" ^ ");\n" ^ 
          "st.global" ^ (generate_ptx_variable_type return_vtype) ^
	  "  [%ptr"^ string_of_int (num_arr_param+3) ^"],  %rtype1" ^ ";\n\n" ^ 
          "RETURN:\n" ^ 
          "\t ret;\n" ^  
          "}\n\n\n"    
      in sprintf " \
