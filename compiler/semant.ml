...
type environment = {
  (* Variables that have been declared in the environment - stores
     variable name, variable type *)
  variable_scope_stack                              : variable_info Variable_Map.t list;
  (* List of kernel functions that have been declared in the
     environment - info from function_info record *)
  kernel_function_map                               : function_info Function_Map.t;
  (* List of host functions that have been declared in the environment
     - info from function_info record *)
...
(* Function for adding initializing host function map, adds builtin
   functions to host function map*)
let init_host_function_map = 
  let fmap = Function_Map.empty in 
  let rec add_functions fmap function_list = 
    match function_list with 
    | [] -> fmap 
    | f_info::tl -> add_functions (Function_Map.add
  ...
  in
  (* let create_built_in_function = (create_function_info Host
     (Ast.Primitive(Ast.Void)) [] [] []) in let
     builtin_function_info_structs = List.map
...
let push_scope env = 
  update_env (Variable_Map.empty :: env.variable_scope_stack)
    env.kernel_function_map env.host_function_map env.is_gpu_env
    env.hof_c_function_list env.hof_ptx_function_list env.expression_stack env.return_lit
...
let pop_scope env = 
  ...
  | local_scope :: tail ->
     update_env tail env.kernel_function_map env.host_function_map
       env.is_gpu_env env.hof_c_function_list env.hof_ptx_function_list
       env.expression_stack env.return_lit

let update_scope updated_scope env = 
  let env = pop_scope env in 
  update_env (updated_scope::env.variable_scope_stack) env.kernel_function_map
    env.host_function_map env.is_gpu_env env.hof_c_function_list
    env.hof_ptx_function_list env.expression_stack env.return_lit

let update_kernel_fmap f_info env = 
  let new_kfmap = Function_Map.add (Utils.idtos(f_info.function_name))
    f_info env.kernel_function_map in
  update_env env.variable_scope_stack new_kfmap env.host_function_map
    env.is_gpu_env env.hof_c_function_list env.hof_ptx_function_list
    env.expression_stack env.return_lit
...
let update_host_fmap f_info env = 
  ...
  update_env env.variable_scope_stack env.kernel_function_map new_hfmap
    env.is_gpu_env env.hof_c_function_list env.hof_ptx_function_list
    env.expression_stack env.return_lit

let update_hof_lists hof_c_fdecl hof_ptx_fdecl env = 
  update_env env.variable_scope_stack env.kernel_function_map env.host_function_map
    env.is_gpu_env (List.rev(hof_c_fdecl::List.rev(env.hof_c_function_list)))
    (List.rev(hof_ptx_fdecl::List.rev(env.hof_ptx_function_list)))
    env.expression_stack env.return_lit

let update_return_lit ptx_lit env = 
  update_env env.variable_scope_stack env.kernel_function_map env.host_function_map
    env.is_gpu_env env.hof_c_function_list env.hof_ptx_function_list  env.expression_stack ptx_lit

let pop_expression_stack env = 
  match env.expression_stack with
  | [] -> env
  | hd::tl -> update_env env.variable_scope_stack env.kernel_function_map
     env.host_function_map env.is_gpu_env  env.hof_c_function_list
...
let get_from_expression_stack nth env =
  if nth > (List.length (List.hd env.expression_stack)) || nth < 0
  then raise Exceptions.Invalid_expression_stack_access
...
let is_function_in_scope id env = 
  ...
  else (Function_Map.mem id env.host_function_map) ||
...
(* Checks a variable declaration and initialization to ensure variable
...
let rec infer_type expression env=
  ...
  | Ast.Identifier_Literal(id) -> 
     if(check_already_declared (Utils.idtos id) env) = false then raise
       (Exceptions.Variable_not_found_in_scope (( Utils.idtos id)) )
  ...
  | Ast.Ternary(e1,e2,e3) ->
     if(same_types (infer_type e1 env) (infer_type e2 env)) = true then infer_type e1 env
     else (raise (Exceptions.Type_mismatch("Ternary doesn't return same type")))
...
let convert_to_c_vdecl vdecl env  = 
  ...
  | Ast.Variable_Declaration(vtype,id) ->
     if(check_already_declared (Utils.idtos(id)) env) = true
     then (raise (Exceptions.Variable_already_declared (Utils.idtos(id))))
     else
       let v_info = { vtype = vtype; register_number = 0; } in
       let new_vmap = Variable_Map.add (Utils.idtos id) v_info
	 (List.hd env.variable_scope_stack) in
...
let convert_to_ptx_param vdecl env =
  ...
  | Ast.Variable_Declaration(vtype,id) -> 
     if(check_already_declared (Utils.idtos id) env) = true
     then raise (raise (Exceptions.Variable_already_declared (Utils.idtos(id))))
     else
       ...
       let v_info ={ vtype = vtype; register_number = reg_num;} in
	 ...
	 (List.hd env.variable_scope_stack) in
       ...
       | Ast.Primitive(p) -> Sast.Ptx_Vdecl(
	 Sast.Param,fst(convert_to_ptx_variable_type vtype env),ptx_id),env
       | Ast.Array(t,n) -> Sast.Ptx_Vdecl(
	 Sast.Param, fst(convert_to_ptx_variable_type vtype env),ptx_id),env
       )
...
let convert_to_ptx_vdecl vdecl env = 
  ...
  | Ast.Variable_Declaration(vtype,id) ->
	 ...
	 (List.hd env.variable_scope_stack) in
       ...
       | Ast.Primitive(Ast.Boolean) -> Sast.Ptx_Vdecl(
	 Sast.Register, fst(convert_to_ptx_variable_type vtype env),ptx_id),env 
       | Ast.Primitive(p) ->Sast.Ptx_Vdecl(
	 Sast.Local,fst(convert_to_ptx_variable_type vtype env),ptx_id),env
       | Ast.Array(t,n) -> Sast.Ptx_Vdecl(
	 Sast.Local,fst(convert_to_ptx_variable_type vtype env),ptx_id),env
       )
...
let get_return_array_info kfunc_id length env = 
  ...
  let return_vtype,env  = convert_to_c_variable_type
...
let make_hof_c_fdecl hof_call env =  
  ...
  | "map" ->
     let kfunc_name = Ast.Identifier(generate_map_ptx_function_name ()) in
     {
       ...
       return_array_info                =  get_return_array_info
	 (Utils.idtos(hof_call.kernel_function_name)) arr_length env;
...
let change_to_ptx_vdecl ckv_info  =
  ...
  in
  incr hof_param_reg_counter;(Sast.Ptx_Vdecl(
    Sast.Global,(get_vtype ckv_info.variable_type),
    (make_ptx_id ckv_info.kernel_name "ptr" !hof_param_reg_counter false false)))
...
let make_hof_ptx_fdecl hof_c_fdecl hof env=  
  let regs = [ convert_to_register_declaration (Sast.Pred) "pred" 2;
               convert_to_register_declaration (Sast.U64) "ptr"
		 (2*((List.length hof.input_arrays)+2));
               convert_to_register_declaration (Sast.S32) "mytid" 2;
               convert_to_register_declaration (Sast.S32) "rtype" 2;
               convert_to_register_declaration (Sast.S32) "vlc"
		 ((List.length hof.input_arrays)+1);
	     ...
	     ] in
  {
    ...
    ptx_higher_order_function_constants  = List.map change_to_ptx_vdecl
      (hof_c_fdecl.higher_order_function_constants);
    ptx_array_length                     = hof_c_fdecl.array_length;
    ptx_input_arrays_info                = List.map change_to_ptx_vdecl
...
let rec convert_to_c_expression e env = 
  ...
  | Ast.Higher_Order_Function_Call(hof) -> 
    ...
    | "map" ->
       ...
       (* Convert *)
       Sast.Function_Call(hof_c_fdecl.higher_order_function_name,
			  (List.map (fun x -> fst(convert_to_c_expression x env))
			     (List.rev hof.input_arrays))),env
    (* | "reduce" -> in Sast.FunctionCall(c_ma) *)
...
let rec is_constant expr = 
  ...
  | Ast.Array_Literal(e_list) -> ((bool_sum (List.map is_constant e_list) 0)
				  = (List.length e_list))
  | _ -> false
...
let rec convert_to_ptx_expression e env = 
  ...
  | Ast.Identifier_Literal(i) -> 
     if(check_already_declared (Utils.idtos i) env) = false
     then raise (Exceptions.Variable_not_found_in_scope ( Utils.idtos i))
     else
       let v_info = get_variable_info (Utils.idtos i) env in
       let is_array = match v_info.vtype with | Ast.Primitive(p) -> false
	 | Ast.Array(t,n) -> true in 
       let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id i (get_reg_type (v_info.vtype))
						   v_info.register_number true is_array) in
       let env = update_expression_stack ptx_lit env in
       Sast.Ptx_Block([Ptx_Empty]),env  
  | Ast.Binop(e1,o,e2) -> 
     if((same_types (infer_type e1 env) (infer_type e2 env)) = false)
     then raise (Exceptions.Type_mismatch "Binop doesn't match")
     else
       (match o with 
       | Ast.And | Ast.Or | Ast.Xor -> 
          (* Check that type is boolean if using boolean operator *)
       ...
       | _ -> 
          (* Check if type is string, array *)
          (match (infer_type e1 env) with
          | Ast.Primitive(t) -> if t = Ast.String then raise
	      (Exceptions.Cannot_perform_operation_on_string (Utils.binary_operator_to_string o))
	    else ()
          | Ast.Array(t,n) -> raise (Exceptions.Cannot_perform_operation_on_array
    ...
    let reg_name,reg_num = generate_reg vtype in
    let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id (
    ...
    let resolve = Sast.Ptx_Binop(ptx_binop,ptx_vtype,ptx_lit,
				 get_from_expression_stack 1 env ,
  ...
  | Ast.Unop(e,o) -> 
     (match o with 
     | Ast.Not -> 
        if((infer_type e env)= Ast.Primitive(Ast.Boolean)) = false
	then raise (Exceptions.Type_mismatch("Must use boolean expression with boolean unop"))
     | _ -> ());
    if((infer_type e env) = (Ast.Primitive (Ast.String)))
    then raise (Exceptions.Cannot_perform_operation_on_string (Utils.unary_operator_to_string o))
    else
      let vtype = infer_type e env in 
      (* Push stack *)
      let env = push_expression_stack env in 
      ...
      let reg_name,reg_num = generate_reg vtype in
      let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id (Ast.Identifier "")
  ...
  | Ast.Array_Literal(e_list) -> 
     ...
     if (valid_array = false) then raise Exceptions.Defg_arrays_must_be_defined_with_constants
       ...
       (* Now we know that the array list is only full of array lits,
	  basically just convert all of them*)
       (* Push on stack *)
       let env = push_expression_stack env in 
       ...
       Sast.Ptx_Block([Ptx_Empty]),env
  | Ast.Function_Call(id, e_list) -> 
     if (is_function_in_scope (Utils.idtos id) env) = false
     then (raise (Exceptions.Function_not_defined (Utils.idtos id)));
    (* Check that function arguments match that of function declaration *)
    let f_info = (get_function_info (Utils.idtos id) env) in
    let f_arg_types = f_info.function_args in 
    ...
    let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id (Ast.Identifier "")
						reg_name reg_num true false) in
    let rec get_elements stack alist = match stack with [] -> alist| hd::tl -> get_elements tl
      (hd::alist) in
    let expr = match rtype with 
      | Ast.Primitive(Ast.Void) -> Sast.Ptx_Empty_Call(
	id,(get_elements(List.hd env.expression_stack) []))
      | _ -> Sast.Ptx_Call(ptx_lit, id, (get_elements(
	List.hd env.expression_stack) []))
    in
    let env = pop_expression_stack env in
    let env = update_expression_stack ptx_lit env in
    expr,env
  ...
  | Ast.Ternary(e1,e2,e3) -> raise Exceptions.C'est_La_Vie
       (*  if(same_types (infer_type e1 env) (infer_type e3 env)) = false
	   then raise (Exceptions.Type_mismatch("Ternary expression don't match"))
        else
          Check e2 is boolean
          if(same_types (infer_type e2 env) (Ast.Primitive(Ast.Boolean))) = false
	   then (raise (Exceptions.Conditional_must_be_a_boolean))
          else *)
...
let convert_to_c_variable_statement vstmt env = 
  ...
  | Ast.Assignment(e1,e2) -> 
     ...
     match e1 with 
     | Ast.Identifier_Literal(id) -> 
        if (check_already_declared (Utils.idtos(id)) env) = false
	then raise (Exceptions.Name_not_found (Utils.idtos id))
        else
       (* Check same types*)
     ...
     | Ast.Array_Accessor(e,e_list,is_lvalue)->
	   ...
	   then raise (Exceptions.Name_not_found (Utils.idtos id))
             ...
             (* Check same types*)
             let arr = get_variable_type (Utils.idtos id) env in
	     ignore(same_types (get_array_el_type arr (List.length e_list)) (infer_type e2 env));
             (*Convert*)
             let c_e1, env = convert_to_c_expression e1 env in 
             let c_e2,env = convert_to_c_expression e2 env in 
             Sast.Assignment(c_e1,c_e2),env
...
let convert_to_ptx_variable_statement vstmt env =
  ...
  | Ast.Initialization(vdecl, e) -> 
     ...
     let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id id (get_reg_type v_info.vtype)
						 v_info.register_number true false) in 
     let env = update_expression_stack ptx_lit env in 
     let ptx_e,env = convert_to_ptx_expression e env in
     (* convert_to_ptx_expression has saved a value in the
        stack. Let us fetch it and resolve*)
     let resolve = Sast.Ptx_Move(fst(convert_to_ptx_variable_type vtype env),
				 get_from_expression_stack 1 env, get_from_expression_stack 0 env)
     in
     (* Pop the stack *)
     let env = pop_expression_stack env in 
  ...
  | Ast.Assignment(e1, e2) ->
     ...
     | Ast.Identifier_Literal(id) -> 
	  ...
	  (Exceptions.Name_not_found (Utils.idtos id))
       ...
       let v_info = get_variable_info (Utils.idtos id) env in
       (* Update vmap *)
       let ptx_id = make_ptx_id id (get_reg_type v_info.vtype) v_info.register_number
	 true false in
       ...
       let resolve = Sast.Ptx_Move(fst(convert_to_ptx_variable_type v_info.vtype env),
				   get_from_expression_stack 1 env,
				   get_from_expression_stack 0 env) in
       (* Pop the stack *)
       let env = pop_expression_stack env in 
	...
	then raise(Exceptions.Name_not_found (Utils.idtos id))
        ...
        (* Check same types*)
        let arr = get_variable_type (Utils.idtos id) env in ignore(same_types
	(get_array_el_type arr (List.length e_list)) (infer_type e2 env));

        (* This case is weird becuase we know e is an identifier literal,
	and that it is an array, so we can gets its information to make a ptx_id
...
let rec convert_to_c_statement stmt env = 
  ...
  | Ast.Block(stmt_list) -> 
     ...
     if (good_statement_order stmt_list) = false
     then raise Exceptions.Have_statements_after_return_break_continue
     else
        (* Convert *)
...
let rec convert_to_ptx_statement stmt env =
  ...
  | Ast.Return(e) ->
     ...
     let expr = Sast.Ptx_Store(Sast.Global,fst(convert_to_ptx_variable_type vtype env),
			       rlit,get_from_expression_stack 0 env) in 
  ...
  (*               let env = push_expression_stack env in 
		   ...
		   let ptx_lit = Sast.Ptx_Identifier_Literal(make_ptx_id (get_reg_type vtype)
		   (get_predicate_counter ()) true false) in
		   let env = update_expression_stack ptx_lit env in
		   let bool_expr = Sast.Block([ptx_e]) in
		   let env = pop_expression_stack env in
		   let branch = Sast.Branch(get_from_expression_stack 0 env,
		   generate_subroutine_name()) in
		   let  
...
let convert_to_c_param vdecl env  = 
  ...
  | Ast.Variable_Declaration(vtype,id) ->
     ...
     then raise (raise (Exceptions.Variable_already_declared (Utils.idtos(id))))
       ...
       in
       let updated_scope = Variable_Map.add (Utils.idtos id) v_info
	 (List.hd env.variable_scope_stack) in
...
let rec check_rtype rtype body env = 
  ...
  | hd::tl-> 
     match hd with 
     | Ast.Return_Void -> if(rtype != Ast.Primitive(Ast.Void))
       then raise Exceptions.Missing_return_type 
       else check_rtype rtype tl env
     | Ast.Return(e) -> 
        if (same_types (infer_type e env) rtype) = false
	then raise Exceptions.Return_type_doesn't_match 
        else check_rtype rtype tl env 
     | _ -> check_rtype rtype tl env

(* Converts from fdecl to c_fdecl *)
let convert_to_c_fdecl fdecl env =
  if (is_function_in_scope (Utils.idtos fdecl.name) env) = true
  then (raise Exceptions.Function_already_declared)
    else
...
let convert ast env =
  let vstmt_list,env                      = convert_list convert_to_c_variable_statement
    (fst(ast)) [] env in
  let ptx_fdecl_list,c_fdecl_list, env    = convert_fdecl_list (snd(ast)) [] [] env in
  let sast                                = (vstmt_list,ptx_fdecl_list,(
    env.hof_ptx_function_list),(env.hof_c_function_list),c_fdecl_list) in 
  sast
