...
type environment = {
  ...
  (* Bool specifying whether environment is being evaluated on the
     gpu *)
  is_gpu_env                                        : bool;
...
(* Used to check function declarations to make sure they aren't
   declaring anything with the same name *)
...
(* TO IMPLEMENT let convert_to_ptx_fdecl fdecl env = (fdecl,env)
   ...
   let host_func_info = { function_type = Host; function_name =
   fdecl.name; function_return_type = fdecl.return_type;
   function_args = fdecl.params; } :: env.host_function_map in
