...
type ptx_identifier = {
  ...
  write_reg 	: bool;   (* Boolean used for codegen to indicate whether we
			     should use the variable name or the register name*)
  is_ptr 		: bool;   (* Just stores if the identifier is a pointer or not.
				     REALLY IMPORTANT *)
}
...
type ptx_fdecl = {
  ...
  register_decls 	  : ptx_register_declaration list; (* Declares the virtual
...
type ptx_higher_order_fdecl = {
  ...
  ptx_higher_order_function_name    : Ast.identifier;	(* Name of this function
							   - ex. map123, map1, map2 *)
  ptx_applied_kernel_function       : Ast.identifier; 	(* Name of kernel function
							   that is called by this function *)
  ptx_higher_order_function_constants 	: ptx_vdecl list; 	(* List of constants
								   passed in that the
								   function can use *)
  ptx_array_length                  : int; 	(* Size of input and return arrays *)
  (* Input array information 
     --If an array has no name (just simply passed in as something like {1,2,3})
     then it is given a temporary generated name *)
...
type c_higher_order_fdecl = {
  ...
  (* Name of kernel function that is called from host (would be
     global void kernel function corresponding to map/reduce) *)
  ...
  array_length 			: int;
  (* Input array information --If an array has no name (just
     simply passed in as something like {1,2,3}) then it is given a
     temporary generated name *)
...
type c_expression =
  ...
  | Array_Accessor of c_expression * c_expression list*
      bool*bool(* Array, indexes, is_lvalue, array_access *)
  | Ternary of c_expression * c_expression * c_expression (* expression if true,
							     condition, expression if false *)
...
type c_statement = 
  ...
  | If of c_expression * c_statement * c_statement (* expression-condition, statement-if block,
