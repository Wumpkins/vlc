...
let data_type_to_string = function 
  | Ast.String -> "string"
  (* | Ast.Byte -> "byte" | Ast.Unsigned_Byte -> "ubyte" *)
  | Ast.Integer -> "int"
...
let rec expression_to_string = function
  ...
  | Ast.Array_Literal(e_list) -> "{" ^ String.concat ","
  ...
  | Ast.Binop(e1, o, e2) -> (expression_to_string e1) ^
     (binary_operator_to_string o) ^ (expression_to_string e2)
  | Ast.Unop(e, o) -> 
     (match o with 
     | Ast.Not -> (unary_operator_to_string o) ^ (expression_to_string e)
     | _ -> (expression_to_string e) ^ (unary_operator_to_string o))
  | Ast.Array_Accessor(e,e_list,b) -> (expression_to_string e) ^ "[" ^
     (String.concat "][" (List.map expression_to_string e_list)) ^ "]"
  | Ast.Ternary(e1,e2,e3) -> (expression_to_string e1) ^ " if(" ^
     (expression_to_string e2) ^ ") else " ^ (expression_to_string e3)
and constant_to_string = function
  | Ast.Constant(id,e) -> (idtos id) ^ "=" ^ (expression_to_string e)
and higher_order_function_call_to_string fcall = (idtos fcall.hof_type) ^ "(" ^
  idtos(fcall.kernel_function_name) ^ "," ^ "consts(" ^
  (String.concat ","(List.map constant_to_string fcall.constants)) ^ ")," ^
  (String.concat "," (List.map expression_to_string fcall.input_arrays)) ^ ")"
...
let rec statement_to_string = function
  ...
  | Ast.If(e,block1, block2) -> 
	...
	"else with " ^(expression_to_string e)^ ":\n" ^ (statement_to_string block2))
  | Ast.While(e,block) -> 
     "while(" ^ (expression_to_string e) ^ "):\n" ^ (statement_to_string block)
  | Ast.For(smtm1, e, smtm2, block) -> "for(" ^ (statement_to_string smtm1) ^ "," ^
     (expression_to_string e) ^ "," ^ (statement_to_string smtm2) ^ "):\n" ^
     (statement_to_string block)
