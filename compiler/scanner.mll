...
let newline = '\n' | "\r\n"
    ...
    | (digit+ '.' digit* | '.' digit+ | digit+ ('.' digit*)? 'e' '-'? digit+
	  | '.' digit+ 'e' '-'? digit+) as fplit { FLOATING_POINT_LITERAL(float_of_string fplit) }
...
and indent = parse
    ...
    | whitespace* as indentation
	{
	  ...
	  if indent_length > stacktop_length then
	    ...
	    let count = 
	      (* Function that pops indent lengths from the stack
		 until we reach the appropriate indent length *)
