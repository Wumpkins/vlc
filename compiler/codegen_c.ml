...
(* Calls generate_func for every element of the list and concatenates
   results with specified concat symbol. Used if you need to generate a
   list of something - e.x. list of statements, list of params *)
...
(* Generates CUDA memory allocation from host to device Fill in with
   VLC_Array*)
let generate_mem_alloc_statement_host_to_device ktype= 
  let arr_length = 
    ...
    | _ -> 1
  in
  sprintf "checkCudaErrors(cuMemAlloc(&" ^ Utils.idtos(ktype.kernel_name)^
    ", sizeof(" ^ (generate_pure_data_type ktype.variable_type) ^ ")*" ^
    string_of_int arr_length ^ "));"

let generate_mem_alloc_host_to_device fcall = 
    (* let rec create_list mylist length element = if length > 0 then
       create_list (element::mylist) (length-1) element else mylist in
    *)let mem_alloc_string =
	String.concat "\n" (List.map generate_mem_alloc_statement_host_to_device
  ...
  let mem_cpy_string = 
    ...
    | Array(t,n) -> 
       let arr_length = 
         ...
         | _ -> 1)
       in
       "checkCudaErrors(cuMemcpyHtoD("^ Utils.idtos(ktype.kernel_name) ^", " ^
	 Utils.idtos(ktype.host_name) ^ ", sizeof(" ^
	 (generate_pure_data_type ktype.variable_type) ^ ")*" ^
	 string_of_int arr_length ^ "));\n"
    | Primitive(t) -> 
       "checkCudaErrors(cuMemcpyHtoD("^ Utils.idtos(ktype.kernel_name) ^", " ^
	 "&" ^ Utils.idtos(ktype.host_name) ^ ", sizeof(" ^
	 (generate_variable_type ktype.variable_type) ^ ")*1" ^ "));\n"
...
let generate_mem_cpy_host_to_device fcall = 
     ...
     *)let mem_cpy_string = String.concat "\n" (List.map
     generate_mem_cpy_statement_host_to_device fcall.input_arrays_info
     ) in
...
let generate_kernel_params arr_info array_length= 
  ...
  let zero_string = "int* ARRAY_LENGTH;\n" ^ "*ARRAY_LENGTH="  ^
    string_of_int array_length ^ ";\n" in
  let params = zero_string ^ "void *KernelParams[] = { &" ^
    kernel_param_string ^", ARRAY_LENGTH};" in
  sprintf "%s" params
...
let rec generate_expression expression  =
  let expr = match expression with
    ...
    | Sast.Binop(e1, o, e2) ->
       (match o with 
       | Sast.Equal | Sast.Not_Equal | Sast.Greater_Than | Sast.Less_Than
       | Sast.Greater_Than_Equal | Sast.Less_Than_Equal -> (generate_expression e1) ^ " " ^
	  (generate_binary_operator o) ^ " " ^ (generate_expression e2)
       | _ -> "("  ^ (generate_expression e1) ^ " " ^ (generate_binary_operator o) ^ " " ^
	  (generate_expression e2) ^ ")"
       )
    | Sast.Unop(e,o) ->
       (match o with 
       | Sast.Not | Sast.Negate  -> "("  ^(generate_unary_operator o) ^
	  (generate_expression e)^ ")"
       | Sast.Plus_Plus | Sast.Minus_Minus -> "(" ^ (generate_expression e) ^
	  (generate_unary_operator o))^ ")"
    | Sast.Array_Accessor(e,e_list,is_lvalue,access_array) -> 
       if is_lvalue = false then
         if access_array = true then
           "("  ^ (generate_expression e) ^ ".get_array_value_host(" ^
	     string_of_int (List.length e_list) ^ "," ^ generate_list generate_expression ","
	     e_list ^ ")" ^ ")"
         else
           "("  ^ (generate_expression e) ^ ".get_element_value(" ^
	     string_of_int (List.length e_list) ^ "," ^ generate_list generate_expression ","
	     e_list ^ ")"^ ")"
       else (generate_expression e)
    | Sast.Ternary(e1,e2,e3) -> "(" ^ (generate_expression e2) ^ ") ? " ^
       (generate_expression e1) ^ ":" ^ (generate_expression e3)
  in sprintf "%s" expr
...
let generate_host_ptr ktype = 
  match ktype.variable_type with
  | Sast.Array(t,n) -> (generate_pure_data_type ktype.variable_type) ^ "* " ^
     Utils.idtos ktype.host_name ^ ";"
  | Sast.Primitive(p) -> (generate_variable_type ktype.variable_type) ^ " " ^
     Utils.idtos ktype.host_name ^ ";"
...
let generate_function_string_read id = 
  "std::string " ^ Utils.idtos id ^ "_str" ^ "((std::istreambuf_iterator<char>(" ^
    Utils.idtos id ^ ")), std::istreambuf_iterator<char>());"
...
let rec generate_if_statements_constants constant_list str index = 
  ...
  | hd::[] -> 
     let statement_string = 
       if index = 0 then
         "if(i ==" ^ string_of_int index ^ "){\n" ^ 
           "\t" ^ Utils.idtos hd.host_name  ^ " = va_arg(constants," ^
	   generate_variable_type hd.variable_type ^ ");\n" ^ 
...
let rec generate_if_statements_input_arrays input_arrays str index = 
  ...
  | hd::[] ->
     let statement_string = 
       if index = 0 then
         "if(i ==" ^ string_of_int index ^ "){\n" ^ 
           "\t" ^ generate_variable_type hd.variable_type ^ " tmp"^ string_of_int index ^
	   " = va_arg(constants," ^ generate_variable_type hd.variable_type ^ ");\n" ^
           "\t" ^ Utils.idtos hd.host_name  ^ " = tmp" ^ string_of_int index ^
	   ".get_values();\n" ^  "}\n"
       else
	 "else{\n" ^ 
           "\t" ^ generate_variable_type hd.variable_type ^ " tmp" ^ string_of_int index ^
  ...
  | hd::tl ->
     let statement_string = 
       if index = 0 then
         "if(i ==" ^ string_of_int index ^ "){\n" ^ 
           "\t" ^ generate_variable_type hd.variable_type ^ " tmp"^
	   string_of_int index ^ " = va_arg(constants," ^ generate_variable_type
	   hd.variable_type ^ ");\n" ^ "\t" ^ Utils.idtos hd.host_name  ^
	   " = tmp" ^ string_of_int index ^ ".get_values();\n" ^ "}\n"
       else
         "else if(i ==" ^ string_of_int index ^ "){\n" ^ 
           "\t" ^ generate_variable_type hd.variable_type ^ " tmp" ^
	   string_of_int index ^ " = va_arg(constants," ^ generate_variable_type
...
let generate_higher_order_function_decl hof = 
  let higher_order_function_decl_string = 
    ...
    | "map" ->  (generate_variable_type hof.return_array_info.variable_type) ^ 
       ...
       (Utils.idtos hof.applied_kernel_function) ^ "_str" ^ " = " ^
       generate_list (fun x-> (Utils.idtos x) ^ "_str") " +\"\\n\" + "
       (List.rev(hof.applied_kernel_function::List.rev(hof.called_functions))) ^ ";\n\n" ^ 
       
       "checkCudaErrors(cuModuleLoadDataEx(&cudaModule," ^
       (Utils.idtos hof.applied_kernel_function) ^ "_str.c_str()" ^ ", 0, 0, 0));\n" ^ 
       "checkCudaErrors(cuModuleGetFunction(&function, cudaModule, \"" ^
       (Utils.idtos hof.applied_kernel_function) ^ "\"));\n\n" ^ "size_t num_constants = "
       ^ string_of_int (List.length hof.higher_order_function_constants) ^ ";\n" ^
       ...
       hof.input_arrays_info ^ "\n"^ 
       generate_list generate_mem_cpy_statement_host_to_device ""
       hof.input_arrays_info ^ "\n\n\n" ^ 
	  ...
	  " va_start(constants,num_constants)\n" ^
	  "for(int i = 0; i < num_constants; i++){\n" ^ 
          (generate_if_statements_for_constants hofcall.constants
	  (List.length hofcall.constants)) ^
	  "}\n" ^ 
	  " va_end(constants);\n" ^
	  ...
	  "}\n" ^  *)
       ...
       (* Launches kernel *)
       "checkCudaErrors(cuLaunchKernel(function, gridSizeX, gridSizeY, gridSizeZ, blockSizeX,
        blockSizeY, blockSizeZ,0, NULL, KernelParams, NULL));\n\n" ^
       (* Copies result array back to host *)
       "checkCudaErrors(cuMemcpyDtoH(" ^ Utils.idtos((hof.return_array_info).host_name) ^ "," ^
       Utils.idtos ((hof.return_array_info).kernel_name) ^ ",
       sizeof(" ^ generate_pure_data_type((hof.return_array_info).variable_type) ^
       ")*" ^ string_of_int hof.array_length ^ "));\n\n" ^ 
       (* Cleanup *)
  ...
  let vstatement_string = match vstatement with
       ...
       | Variable_Declaration(vtype,id) ->
          ...
          | Sast.Array(t,i_list) -> 
             (generate_vdecl d) ^ "= VLC_Array<"^ generate_variable_type t^ ">(" ^ 
               ...
               string_of_int (List.length i_list) ^ "," ^ generate_list string_of_int ","
	       i_list ^ ");\n"
    ...
    | Sast.Assignment (e1, e2) -> 
       ...
       | Sast.Array_Accessor(e,e_list,is_lvalue,array_access) ->
          if array_access = true then (generate_expression e1) ^ ".set_array_value(" ^
	    (generate_expression e2) ^ "," ^ string_of_int (List.length e_list) ^ ","^
	    (generate_list generate_expression "," e_list )^");\n"
          else (generate_expression e1) ^ ".set_element_value(" ^ (generate_expression e2) ^
	    "," ^ string_of_int (List.length e_list)^ ");\n"
...
let rec generate_statement statement  =
  let statement_string = match statement with
    ...
    | Sast.For(stmt1,e,stmt2,stmt3) -> "for(" ^ (generate_for_statement stmt1)^";"^
       (generate_expression e) ^ ";" ^ (generate_for_statement stmt2) ^ "){\n" ^
