...
VLC_Array<T>::VLC_Array(const VLC_Array<T> &vlcarray){
  ...
  for(size_t i = 0; i < this->num_dimensions;i++) {
    this->dimensions[i] = vlcarray.get_dimensions()[i];
  }
}
...
VLC_Array<T> VLC_Array<T>::operator=(const VLC_Array<T> &vlcarray){
  ...
  /* Now access the values that are passed in */
  for(size_t j = 0; j < this->num_values;j++){
    this->values[int(j)]= vlcarray.get_values()[int(j)];
  }
  for(size_t i = 0; i < this->num_dimensions; i++){
    this->dimensions[int(i)]= vlcarray.get_dimensions()[int(i)];
  }
  return *this;
}
